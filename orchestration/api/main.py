"""
Orchestration Social API — upvotes, trending, and comments.
...
DB_PATH = os.environ.get(
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "orchestration.db"),
)
...
app = FastAPI(title="Orchestration API", docs_url=None, redoc_url=None)
...
@app.get("/orchestration/api/votes")
def get_all_votes():
...
@app.post("/orchestration/api/vote/{pattern_id}")
...
@app.get("/orchestration/api/comments/{pattern_id}")
...
@app.post("/orchestration/api/comments/{pattern_id}")
...
@app.post("/orchestration/api/comments/{comment_id}/flag")
...
@app.get("/orchestration/api/health")
def health():
    return {"status": "ok"}
