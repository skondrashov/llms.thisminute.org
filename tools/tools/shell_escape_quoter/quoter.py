...
class SpecialChar:
    ...
    exploit_example: str = ""  # concrete example of what can go wrong
...
_POSIX_BASE_CHARS = (
    SpecialChar("'", "single quote", "Ends single-quoted string; can break out of quoting context", "high", "injection",
                "echo 'hello' ; rm -rf /' -- attacker closes your quote and injects a command"),
...
_FISH_CHARS = (
    ...
    SpecialChar("$", "dollar sign", "Variable expansion: $VAR substitutes the variable's value", "high", "expansion",
                "echo $HOME -- expands to the user's home directory; fish uses $var not ${var}"),
    SpecialChar("\\", "backslash", "Escape character: removes special meaning of next character", "medium", "control",
                "echo hello\\ world -- backslash changes how the next character is interpreted"),
    SpecialChar("\n", "newline", "Command separator", "high", "separator",
                "echo safe\\nrm -rf / -- newline starts a second command"),
    SpecialChar(";", "semicolon", "Command separator", "high", "separator",
                "echo hi ; rm -rf / -- semicolon chains a destructive command"),
    SpecialChar("&", "ampersand", "Background execution (&)", "high", "separator",
                "echo hi & rm -rf / -- runs destructive command in background"),
    SpecialChar("|", "pipe", "Pipe output to next command", "high", "separator",
                "echo hi | sh -- pipes output to shell for execution"),
    SpecialChar("<", "less-than", "Input redirection", "medium", "redirect",
                "cmd < /etc/passwd -- reads sensitive file as input"),
    SpecialChar(">", "greater-than", "Output redirection", "medium", "redirect",
                "echo pwned > ~/.config/fish/config.fish -- overwrites fish config"),
    SpecialChar("(", "open paren", "Command substitution: (cmd) in fish", "medium", "injection",
                "echo (rm -rf /) -- fish uses (cmd) instead of $(cmd) for command substitution"),
    SpecialChar(")", "close paren", "Ends command substitution", "medium", "injection",
                "Closes command substitution started with ("),
...
_POWERSHELL_CHARS = (
    ...
    SpecialChar(">", "greater-than", "Redirection operator", "medium", "redirect",
                "'pwned' > $PROFILE -- overwrites your PowerShell profile"),
    SpecialChar("(", "open paren", "Subexpression, grouping", "medium", "injection",
                "(Remove-Item /) -- groups a destructive command"),
    SpecialChar(")", "close paren", "Ends subexpression", "medium", "injection",
                "Closes a subexpression started with ("),
    SpecialChar("{", "open brace", "Script block delimiter", "medium", "injection",
                "& {malicious} -- script blocks contain executable code"),
    SpecialChar("}", "close brace", "Ends script block", "medium", "injection",
                "Closes a script block started with {"),
    SpecialChar("@", "at sign", "Splatting (@args), array literal @(), hash table @{}", "medium", "expansion",
                "@params -- splatting passes a hashtable as named parameters to a command"),
    SpecialChar("#", "hash", "Comment", "medium", "control",
                "# rest of line is ignored -- attacker can hide malicious code after visible text"),
    SpecialChar("*", "asterisk", "Wildcard in paths", "medium", "glob",
                "Remove-Item *.txt -- wildcard matches all .txt files"),
    SpecialChar("?", "question mark", "Wildcard: single character; also alias for Where-Object", "medium", "glob",
                "Get-Item file?.txt -- matches file1.txt, file2.txt, etc."),
    SpecialChar("[", "open bracket", "Type literal, indexing", "low", "expansion",
                "[System.IO.File]::ReadAllText('secret.txt') -- type literals access .NET classes"),
    SpecialChar("]", "close bracket", "Ends type literal or indexing", "low", "expansion",
                "Closes a type literal started with ["),
    SpecialChar(" ", "space", "Parameter separator", "low", "separator",
                "Remove-Item my file.txt -- splits into two arguments: 'my' and 'file.txt'"),
    SpecialChar("\t", "tab", "Parameter separator", "low", "separator",
                "Like space, tab splits parameters unexpectedly"),
    SpecialChar("\r", "carriage return", "Can manipulate terminal display", "medium", "control",
                "Carriage return can overwrite visible terminal output to hide real content"),
    SpecialChar("\x00", "null byte", "Can cause unexpected string termination", "high", "control",
                "Null byte may truncate strings at the C level, dropping trailing content"),
)

_CMD_CHARS = (
    SpecialChar('"', "double quote", "Quoting mechanism; toggles quote mode on/off", "high", "injection",
                'echo "hello" & del /q * -- attacker closes quote and injects a destructive command'),
    SpecialChar("^", "caret", "Escape character in cmd.exe: ^c removes special meaning of c", "medium", "control",
                "echo hello^&world -- ^ removes the special meaning of &, but misuse can bypass escaping"),
    SpecialChar("&", "ampersand", "Command separator (cmd1 & cmd2), conditional (&& and ||)", "high", "separator",
                "echo hi & del /q * -- ampersand chains a destructive command"),
    SpecialChar("|", "pipe", "Pipe output to next command", "high", "separator",
                "echo data | cmd -- pipes output to cmd for execution"),
    SpecialChar("<", "less-than", "Input redirection", "medium", "redirect",
                "cmd < secret.txt -- reads a sensitive file as input"),
    SpecialChar(">", "greater-than", "Output redirection", "medium", "redirect",
                "echo pwned > autoexec.bat -- overwrites a file with attacker content"),
    SpecialChar("(", "open paren", "Command grouping", "medium", "injection",
                "(del /q * & echo done) -- groups destructive commands together"),
    SpecialChar(")", "close paren", "End command grouping", "medium", "injection",
                "Closes command group started with ("),
    SpecialChar("%", "percent", "Variable expansion: %VAR%, FOR loop variable %%i", "high", "expansion",
                "%USERNAME% -- expands to current user; %PATH% exposes system configuration"),
    SpecialChar("!", "exclamation", "Delayed variable expansion: !VAR! when enabled", "high", "expansion",
                "!USERNAME! -- with delayed expansion enabled, expands variables at execution time"),
    SpecialChar("\n", "newline", "Command separator", "high", "separator",
                "echo safe\\nformat C: -- newline starts a new command"),
    SpecialChar("\r", "carriage return", "Can manipulate display output", "medium", "control",
                "Carriage return can overwrite visible output in the terminal"),
    SpecialChar(" ", "space", "Argument separator", "low", "separator",
                "del my file.txt -- splits into two arguments: 'my' and 'file.txt'"),
    SpecialChar("\t", "tab", "Argument separator", "low", "separator",
                "Like space, tab splits arguments unexpectedly"),
    SpecialChar("*", "asterisk", "Wildcard in some contexts", "low", "glob",
                "del *.txt -- wildcard matches all .txt files in the directory"),
    SpecialChar("?", "question mark", "Wildcard: single character", "low", "glob",
                "del file?.txt -- matches file1.txt, file2.txt, etc."),
    SpecialChar("\x00", "null byte", "String terminator; truncates strings", "high", "control",
                "Null byte silently truncates everything after it"),
)
...
def main():
    ...
    parser.add_argument("--explain", action="store_true",
                        help="Show exploit examples for each dangerous character (use with --analyze).")
    ...
    if args.analyze:
        ...
        for sa in result.shell_analyses:
            ...
            if getattr(args, "explain", False) and sa.findings:
                seen = set()
                for finding in sa.findings:
                    if finding.char in seen:
                        continue
                    seen.add(finding.char)
                    info = finding.char_info
                    char_display = repr(finding.char) if ord(finding.char) < 0x20 or ord(finding.char) == 0x7f else finding.char
                    print(f"    [{info.danger_level:>6}] pos {finding.position}: {char_display} - {info.description}")
                    if info.exploit_example:
                        print(f"           -> {info.exploit_example}")
